...
class DrawOnTop extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        System.out.println(mImageHeight+" & "+mImageWidth);
        // Calculate histogram
        ...
        int left = getLeft()+(getRight()-((getBottom()-getTop())/3))/2;
        int right = getRight()-(getRight()-((getBottom()-getTop())/3))/2;
        int top = getTop()+(getBottom()-getTop())/3;
        int bottom = getBottom()-(getBottom()-getTop())/3;

        //detect tomatoe
        if (imageRedMean < 255 && imageRedMean > 170 && imageGreenMean > 0 && imageGreenMean < 70 & imageBlueMean > 0 && imageBlueMean < 70){
